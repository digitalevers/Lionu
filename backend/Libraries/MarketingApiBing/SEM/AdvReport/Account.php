...
class Account {
    ...
    //报表需要返回的数据列
    private $columns = [
        "AllConversions","Impressions","Clicks","AccountId","Spend","TimePeriod","DeviceType"
    ];
    ...
    private function _getReportId($devToken, $authToken, $aid, $startDate, $endDate){
        ...
        //dump($reportRes);
        //exit;
        if(isset($reportRes['OperationErrors'])){
            $error = $reportRes['OperationErrors'][0];
            $exceptionMsg = isset($this->errorMsg[$error['Code']]) ? $this->errorMsg[$error['Code']].' '.$error['ErrorCode'] : $error['ErrorCode'];
            throw new \Exception($exceptionMsg, $error['Code']);
        }
